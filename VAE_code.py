...
model.eval()
with torch.no_grad():
    mu_train, _ = model.encode(X_train_tensor)
    X_train_latent = mu_train.to(device)
    mu_test, _ = model.encode(X_test_tensor)
    X_test_latent = mu_test.cpu()
...
plot_path = f'/home/tl545/桌面/iibtl545/datasets/results/VAE_results/latent_space_true_labels_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
# plt.savefig(plot_path)
print(f"Latent space plot saved at {plot_path}")

# Save results
output_file_path = f'/home/tl545/桌面/iibtl545/datasets/results/VAE(mean)_report/VAE_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
testing_data['Predicted_Label'] = predicted_labels
