...
#training_data_path = '/home/tl545/桌面/iibtl545/datasets/training+testing/training&testing(6000 new)/photocurrent_results_random_all_training_6000.csv'
#testing_data_path = '/home/tl545/桌面/iibtl545/datasets/training+testing/training&testing(6000 new)/photocurrent_results_random_all_testing_6000.csv'
training_data_path = '/home/tl545/桌面/iibtl545/datasets/training+testing/train_new.csv'
testing_data_path = '/home/tl545/桌面/iibtl545/datasets/training+testing/test_new.csv'
...
y_train_tensor = torch.tensor(y_train - (1 if MLP_mode == "filtered_materials" else 0), dtype=torch.long).to(device)
...
for run in range(num_runs):
    ...
    model = MLP(input_size, hidden_sizes, output_size).to(device)  # Move model to GPU
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, X_train_tensor.size(0), batch_size):
            ...
            # Verify execution on GPU for tensors and model
            if epoch == 0 and run == 0 and i == 0:  # Only check during the first iteration
                print(f"Batch X is on: {batch_X.device}")
                print(f"Model weights are on: {model.network[0].weight.device}")
    ...
    X_test_tensor = torch.tensor(X_test, dtype=torch.float32).to(device)  # Move testing data to GPU

    model.eval()
    with torch.no_grad():
        test_outputs = model(X_test_tensor)
        predicted_labels = torch.argmax(test_outputs, dim=1).cpu().numpy()  # Move predictions back to CPU for processing
...
output_file_path = f'/home/tl545/\u684c\u9762/iibtl545/datasets/results/predictions_{MLP_mode}_{code_time}_accuracy{accuracy:.4f}.xlsx'
