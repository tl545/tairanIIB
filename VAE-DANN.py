...
class DANN(nn.Module):
    ...
    def forward(self, x, alpha):
        x = self.noise(x)
        features, x_recon, mu, logvar = self.encoder_module(x)
        ...
        return class_outputs, domain_outputs, x_recon, mu, logvar
...
def train_dann(model, train_loader, val_loader, device, optimizer, criterion_cls, criterion_domain):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (x_batch, y_batch, d_batch) in enumerate(train_loader):
            ...
            class_outputs, domain_outputs, x_recon, mu, logvar = model(x_mix, alpha)
            ...
            total_loss = loss_cls + loss_domain + recon_weight * loss_recon + 0.1 * loss_kl
            total_loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for x_val, y_val, _ in val_loader:
                ...
                preds, _, _, _, _ = model(x_val, alpha=0.0)
...
def evaluate(model, X_test, y_test, device, scaler):
    ...
    with torch.no_grad():
        ...
        for _ in range(n_votes):
            ...
            logits, _, _, _, _ = model(noisy_input, alpha=0.0)
